...
namespace AssetInventory
{
    public sealed class UnityPackageImporter : AssetImporter
    {
        ...
        public async Task IndexDetails(int assetId = 0)
        {
            ...
            if (assetId == 0)
            {
                assets = DBAdapter.DB.Table<Asset>().Where(asset => !asset.Exclude && (asset.CurrentState == Asset.State.InProcess || asset.CurrentState == Asset.State.SubInProcess) && (asset.AssetSource == Asset.Source.AssetStorePackage || asset.AssetSource == Asset.Source.CustomPackage)).ToList();
            }
            else
            {
